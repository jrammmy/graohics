...
var VSHADER_SOURCE =
  ...
  '  gl_Position = u_ProjMatrix * a_Position;\n' +  //setting gl position ->u_Proj...*a_Pos...
  '  v_Color = a_Color + ambient_Color;\n' +                        //setting v_Color->a_Color
  '}\n';
...
var FSHADER_SOURCE =                  
  ...
  'void main() {\n' +                               //our main function start
  '  gl_FragColor = v_Color;\n' +                   //setting gl_FragColor->v_Color ; Linked to v_Color above
  '}\n';
...
function setUpOpenGL(gl)
{
  ...
  gl.uniform4f(ambient_Color, 0.0, 0.0, 0.2, 1.0);
}
...
function click(ev, gl, canvas, a_Position, a_Color) 
{
  ...
  else if(linePoints.length > 0 || ev.button != 2)
  {
    ...
    gl.bufferData(gl.ARRAY_BUFFER, vertexHolder, gl.STATIC_DRAW);
...
function drawCylinderObjects(a_Position, gl, a_Color) 
{
  ...
  for(var i = 0; i < indexHolder.length; i+=3)
  {
    console.log('Triangle between vertices: ' + indexHolder[i] + ', ' + indexHolder[i+1] + ', ' + indexHolder[i+2]);
    console.log('Color of Vertex ' + indexHolder[i] + ': ' + colorHolder[(indexHolder[i]*3)] + ', ' + colorHolder[(indexHolder[i]*3)+1] + ', ' + colorHolder[(indexHolder[i]*3)+2]);
    console.log('Color of Vertex ' + indexHolder[i+1] + ': ' + colorHolder[(indexHolder[i+1]*3)] + ', ' + colorHolder[(indexHolder[i+1]*3)+1] + ', ' + colorHolder[(indexHolder[i+1]*3)+2]);
    console.log('Color of Vertex ' + indexHolder[i+2] + ': ' + colorHolder[(indexHolder[i+2]*3)] + ', ' + colorHolder[(indexHolder[i+2]*3)+1] + ', ' + colorHolder[(indexHolder[i+2]*3)+2]);
  }
  ...
  screenRefreshCylinders(gl, a_Position, n, indexHolderWON.length, SIZE)
}
...
// Retrieve <canvas> element by name
  ...
  screenRefreshCylinders(gl, a_Position, indexHolder.length, indexHolderWON.length, SIZE);
}
...
function generateSmoothColor(surfaceNum, vertexCt)
{
  ...
  for(var i = 0; i < shapes2.length; i++)
  {
    for (var j = 0; j < shapes2[i].length; j++)
    {
      ...
      l = new Vector3([(1-shapes2[i][j].elements[0]), (1-shapes2[i][j].elements[1]), (1-shapes2[i][j].elements[2])]);
      l.normalize();
...
function ambientSwitch()
{
  ...
  {
    ambient = false;
    gl.uniform4f(ambient_Color, 0.0, 0.0, 0.0, 1.0);
  }
  else
  {
    ambient = true;
